...
public class InitController {
    ...
    public ModelAndView start(AbsSender sender, Session session,
        ...
        if (admins.contains(user.getUserName())) {
            String adminChatId = storage.getChatId(user.getUserName());
            if (adminChatId == null || !adminChatId.equals(chatId)) {
                storage.update(user.getUserName(), chatId);
                sender.execute(SendMessage.builder().text("Сохранил адрес чата").chatId(chatId).build());
            } else {
                sender.execute(SendMessage.builder().text("Все в порядке").chatId(chatId).build());
            }
            session.stop();
